...
@Layer1StrategyName("VWAP Breakout ATR TP/SL")
@Layer1ApiVersion(Layer1ApiVersionValue.VERSION2)
public class ATROnlyVwap implements CustomModule, TradeDataListener, TimeListener, OrdersListener, CustomSettingsPanelProvider, BarDataListener {

    private VWAPCalculator vwapCalculator = new VWAPCalculator();
    private OrderPlacerATR orderPlacer;
    ...
    public void initialize(String alias, InstrumentInfo info, Api api, InitialState initialState) {
        ...
        orderPlacer = new OrderPlacerATR(alias, api);
    }
    ...
    private void checkForBreakoutOrBreakdown(double price, double vwapValue, long dynamicConfirmationTime) {
        ...
        if (isBullishBreakout || isBearishBreakdown) {
            if (lastCrossTime == 0) {
                ...
                double atr = calculateATR();
                if (!Double.isNaN(atr)) {
                    orderPlacer.placeOrder(isBullishBreakout, price, quantity, trailingStopStep, atr);
                }
            }
        } else {
            lastCrossTime = 0;
